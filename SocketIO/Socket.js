...
module.exports = function(io){
  io.on("connect", (socket) => {
    ...
    socket.on('deleteBill',(billID)=>{
      billController.deleteBill(billID).then(()=>{
        billController.getAllBill().then((allBill)=>{
          socket.emit('deleteBillResult',true)
          socket.emit("allBillResult",allBill)
          socket.broadcast.emit("allBillResult",allBill)
        })
      })
      .catch((err)=>{
        console.log(err)
        socket.emit('deleteBillResult',false)
      })
    })
    ...
    socket.on('switchTable',(bill_id,chosenTable)=>{
      billController.switchTable(bill_id,chosenTable).then(()=>{
        billController.getAllBill().then((allBill)=>{
          socket.emit('switchTableResult',true)
          socket.emit("allBillResult",allBill)
          socket.broadcast.emit("allBillResult",allBill)
        })
      })
      .catch((err)=>{
        console.log(err)
        socket.emit('switchTableResult',false)
      })
    })
  });
} 
